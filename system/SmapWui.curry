...
wSmapSelect :: String -> (a -> String) -> [a] -> WuiSpec a
...
wSmap7Tuple :: WuiSpec a -> WuiSpec b -> WuiSpec c -> WuiSpec d -> WuiSpec e
...
renderSmapString (state,label,icon,typeVal,focus,pHolder,help,err) [hExp] =
  ...
  where 
    mInputGroup 
      ...
      | otherwise = div [class "input-group input-group-sm"]
                      [span [class "input-group-addon"]
                        [glyphicon icon]
                        ,mAutofocus inputField]
    mHelpBlock
      | null help = empty
      | otherwise = span [class "help-block"] [text help]
    mAutofocus
...
greyWuiBtn (WHandler hdlr) label =
  submitButton [class "btn btn-default"] hdlr label
...
blueWuiBtn (WHandler hdlr) label =
  submitButton [class "btn btn-primary"] hdlr label
