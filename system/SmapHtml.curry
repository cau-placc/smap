...
renderNavbar url langNames mAuthNData =
  nav [class "navbar navbar-inverse navbar-fixed-top",role "navigation"]
    [container
      ...
      ,div [class "collapse navbar-collapse"]
        -- Quick search form
        [(div [class "navbar-form navbar-left",title navbarSearchTooltip]
        ...
        ,ul [class "nav navbar-nav navbar-right"]
          ...
          (\(userName,userIsAdmin) ->
           li [class "dropdown"]
            [a 
              [href "#"
              ,class $ "dropdown-toggle"++(mAdmin userIsAdmin)
              ,dropdownToggle]
  ...
  where
    ...
    adminMenu =
      [li [class "divider"] []
      ,li [class "dropdown-header"] [text "Admin options"]
      ,li []
...
smTextInput pholder = 
  input [("type","text"),class "form-control input-sm",placeholder pholder]
...
greyLinkBtn url = a [href url,class "btn btn-default"]

--- A hyperlink rendered as a small blue button.
--- @param url   - the hyperlink URL 
--- @param label - label HTML expressions
smBlueLinkBtn :: String -> [HtmlExp] -> HtmlExp
smBlueLinkBtn url = a [href url,class "btn btn-primary btn-sm"]

--- A hyperlink rendered as a blue button.
--- @param url   - the hyperlink URL 
--- @param label - label HTML expressions
blueLinkBtn :: String -> [HtmlExp] -> HtmlExp
blueLinkBtn url = a [href url,class "btn btn-primary"]
...
blueSubmitBtn = submitButton [class "btn btn-primary"]

--- A submit button rendered as a orange button.
--- @param hdlr  - the HTML handler
--- @param label - label HTML expressions
orangeSubmitBtn :: HtmlHandler -> [HtmlExp] -> HtmlExp
orangeSubmitBtn = submitButton [class "btn btn-warning"]

--- A submit button rendered as a link button.
--- @param hdlr  - the HTML handler
--- @param label - label HTML expressions
linkSubmitBtn :: HtmlHandler -> [HtmlExp] -> HtmlExp
linkSubmitBtn = submitButton [class "btn btn-link"]

-- Standard buttons

--- A grey cancel button.
greyCancelBtn :: HtmlExp
greyCancelBtn = a [href landingPageUrl,class "btn btn-default"] [text "Cancel"]
